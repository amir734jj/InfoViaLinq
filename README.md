...
```csharp
IInfoViaLinq<Person> _utility = new InfoViaLinq<Person>();
...
_utility.PropLambda(x => x.Parents.GreatParents.Parents.FatherName).Members().Last().GetCustomAttribute<DisplayAttribute>();
...
```csharp
var str = new InfoViaLinq<string>().FuncLambda<string>(x => x.ToString).GetMethodInfo().Name;
// "ToString"

var str = new InfoViaLinq<string>().FuncLambda<string, int>(x => x.CompareTo).GetMethodInfo().Name;
// "CompareTo"
```
